...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .log-action { color: var(--text); line-height: 1.5; }
  .log-card {
    background: var(--surface); border: 1px solid var(--border);
...
async function loadWatchlist(gid) {
  ...
  try {
    ...
    document.getElementById('watchlist-sub').textContent =
      `${data.name} — ${data.watchlist.length} คน`;
    ...
    el.innerHTML = `
      ...
      <div class="watch-list" id="watch-list">
        ${list.length
          ? list.map(uid => watchItem(uid, gid)).join('')
...
function watchItem(uid, gid) {
  return `<div class="watch-item" id="wi_${uid}">
    <span class="watch-uid">👤 ${escHtml(uid)}</span>
...
async function addToWatchlist() {
  ...
  if (list) {
    ...
    el.innerHTML = watchItem(uid, currentGuild.id);
    list.prepend(el.firstChild);
  }
  showToast(`✅ เพิ่ม ${uid} แล้ว`, 'success');
}
        ...
        <tbody>
          ${logs.map(({ uid, line }) => {
            ...
            return `<tr>
              ...
              <td class="log-uid" onclick="filterByUid('${escHtml(uid)}')" title="คลิกเพื่อกรองเฉพาะ user นี้">${escHtml(uid)}</td>
